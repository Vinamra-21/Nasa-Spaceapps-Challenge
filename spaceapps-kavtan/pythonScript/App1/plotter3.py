...
def calculate_coordinates(lat, lon, distance_km=5):
    R = 6371  
...
def create_aoi_polygon(latitude, longitude, aoi_render_distance):
    lat1, lat2, lon1, lon2 = calculate_coordinates(latitude, longitude, aoi_render_distance)
    return {
        "type": "Feature",
        "properties": {},
        "geometry": {
...
aoi_render_distance=int(input('Enter the aoi distance to render: '))
latitude, longitude = get_coordinates(location)
aoi = create_aoi_polygon(latitude, longitude, aoi_render_distance)
...
plt.title(f"Population Density over {location} (2000-2020)")
